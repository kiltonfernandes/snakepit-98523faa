...
import { ArrowDown, ArrowUp, ArrowUpDown, ChevronDown, GripVertical, Plus, Trash2 } from 'lucide-react';
import { SORT_FIELD_LABELS, SortField, SortRule } from '@/lib/releases-grouping';
...
export function SortRulesPopover({
  ...
  const add = () => {
    ...
    const next = FIELDS.find(f => !used.has(f)) || 'release_date';
    onChange([...rules, { field: next, dir: 'desc' }]);
  };
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={rules.length ? 'secondary' : 'outline'} size="sm" className="gap-1.5 h-8">
          ...
          Sort {rules.length > 0 && <span className="text-muted-foreground">({rules.length})</span>}
      ...
      <PopoverContent className="w-[420px]" align="end">
        <div className="space-y-2">
          ...
          {rules.length === 0 && (
            <p className="text-xs text-muted-foreground py-2">Nenhuma regra. Adicione um nível abaixo.</p>
          )}
          {rules.map((r, idx) => (
            <div key={idx} className="flex items-center gap-1.5">
              ...
              <Select value={r.field} onValueChange={v => update(idx, { field: v as SortField })}>
                <SelectTrigger className="h-8 flex-1"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {FIELDS.map(f => (
                    <SelectItem key={f} value={f}>{SORT_FIELD_LABELS[f]}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <Button variant="outline" size="sm" className="w-full gap-1.5 mt-2" onClick={add}>
            <Plus className="h-3.5 w-3.5" /> Adicionar nível de ordenação
          </Button>
        </div>
      </PopoverContent>
    </Popover>
